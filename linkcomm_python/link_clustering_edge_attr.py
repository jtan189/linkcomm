...
class HLC:
    ...
    def single_linkage(self, threshold=None, w=None, dendro_flag=False, ea=None):
        print "clustering..."
        ...
        if w == None: # unweighted
            if ea != None:
                H = similarities_unweighted( self.adj, ea ) # min-heap ordered by 1-s
            else:
...
def similarities_unweighted(adj, ea):
...
"""
    ...
    parser.add_option("-a", "--attribute-file", dest="attrib_file", default=None,
                      help="filename of edge attributes file")
    ...
    attrib_filename = options.attrib_file
    ...
    attrib_file = open(attrib_filename, 'rb')
    edge_attribs = [row.strip().split(' ') for row in attrib_file]
    ...
    if threshold is not None:
        if is_weighted:
            ...
            edge2cid,D_thr = HLC( adj,edges ).single_linkage( threshold, ea=edge_attribs )
        print "# D_thr = %f" % D_thr
        ...
        if is_weighted:
            ...
            if dendro_flag:
                edge2cid,S_max,D_max,list_D, orig_cid2edge, linkage = HLC( adj,edges ).single_linkage( 
                                                                                dendro_flag=dendro_flag, ea=edge_attribs )
                write_dendro( "%s_dendro" % basename, orig_cid2edge, linkage)
            else:
                edge2cid,S_max,D_max,list_D = HLC( adj,edges ).single_linkage(ea=edge_attribs)
